...
// --- clipboard store ---
let clipboard = { content: '', type: 'text' as 'text' | 'image', deviceId: '', updatedAt: 0 };
...
async function authOwnerOrDevice(req: http.IncomingMessage): Promise<{ owner: boolean; device: string | null }> {
  if (authorized(req)) return { owner: true, device: null };
  const presented = bearerToken(req);
  if (!presented) return { owner: false, device: null };
  const inRegistry = await findDeviceByToken(presented);
  if (inRegistry) return { owner: false, device: inRegistry.name };
  const pending = pendingDeviceNameByToken(presented);
  if (pending) return { owner: false, device: pending };
  return { owner: false, device: null };
}

/** 设备 term 端点：agent 端口 +10（约定），/mcp → /term<子路径> */
function termUrlFromEndpoint(mcpUrl: string, subPath: string): string {
  try {
...
function readBody(req: http.IncomingMessage, maxBytes = 65536): Promise<string> {
...
async function readJson(req: http.IncomingMessage, maxBytes = 65536): Promise<any> {
  try {
...
function isBlockedIpv6(host: string): boolean {
  ...
  if (/^(fe80|f[c-d]|ff)/i.test(host)) return true; // link-local / ULA / multicast
...
export async function safeUrl(raw: string): Promise<string | null> {
  let u: URL;
  try {
    u = new URL(raw);
  } catch {
  ...
  try {
    ...
    for (const a of addrs) {
      if (a.family === 4 ? isBlockedIpv4(a.address) : isBlockedIpv6(a.address)) return null;
    }
  } catch {
    return null;
  }
...
async function finalizeDeviceRegistration(op: { operation: string; device: string; deviceIp?: string; devicePort?: number; deviceToken?: string; machineId?: string }): Promise<{ registered: boolean; error?: string }> {
  if (op.operation !== 'device_registration') return { registered: true };
...
export function startServer(port: number = serverConfig.port): http.Server {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (method === 'POST' && url === '/pair/init') {
      ...
      if (!pairInitLimiter.allow(clientIp(req), 30)) return json(res, 429, { error: 'too many requests' });
      const code = mintCode();
    ...
    if (method === 'POST' && url === '/pair/claim') {
      if (!pairClaimLimiter.allow(clientIp(req), 10)) return json(res, 429, { error: 'too many attempts' });
    ...
    if (method === 'POST' && revokeMatch) {
      ...
      try {
        ...
        if (!removed) return json(res, 404, { error: 'device not found' });
    ...
    if (method === 'POST' && renameMatch) {
      ...
      try {
        ...
        if (!renamed) return json(res, 404, { error: 'device not found' });
    ...
    if (method === 'POST' && reurlMatch) {
      ...
      const body = await readJson(req);
      const { newUrl } = body;
      if (!newUrl) return json(res, 400, { error: 'newUrl required' });
      ...
      if (!u) return json(res, 400, { error: 'invalid URL: http(s) only, no loopback/private' });
      try {
        const name = safeDecode(reurlMatch[1]);
        const updated = await updateDeviceUrl(name, u);
        if (!updated) return json(res, 404, { error: 'device not found' });
    ...
    if ((method === 'POST' || termGet) && deviceProxy) {
      ...
      try {
        ...
        if (!endpoint) return json(res, 404, { error: 'device not found' });
        ...
        if (upSession) headers['mcp-session-id'] = upSession;
        ...
        if (!res.destroyed) return json(res, 502, { error: err instanceof Error ? err.message : String(err) });
      }
      return;
    }
    ...
    if (method === 'POST' && url === '/clipboard/push') {
      ...
      if (!auth.owner && !auth.device) return json(res, 401, { error: 'unauthorized' });
      let text: string;
      try {
    ...
    if (method === 'POST' && url === '/ops/request') {
      ...
      if (!operation) return json(res, 400, { error: 'operation required' });
    ...
    if (method === 'POST' && url === '/ops/card-action') {
      if (!TOKEN) return json(res, 403, { error: 'card-action disabled without owner token (open mode)' });
      const ip = clientIp(req);
      if (ip !== '127.0.0.1' && ip !== '::1') return json(res, 403, { error: 'loopback only' });
      if (!approveLimiter.allow(ip, 30) || !approveGlobalLimiter.allow('global', 300)) {
        return json(res, 429, { error: 'too many attempts' });
      }
      const body = await readJson(req);
      const { opId, action, signature, senderId } = body;
      if (!opId || !signature || !senderId) return json(res, 400, { error: 'opId/signature/senderId required' });
    ...
    if (method === 'POST' && url === '/ops/reject') {
      ...
      if (ok) {
        ...
        if (op) void updateApprovalCard(op, 'rejected').catch(() => {});
      }
    ...
    if (method === 'GET' && opsMatch) {
      ...
      const auth = await authOwnerOrDevice(req);
      if (!auth.owner && auth.device !== op.device) return json(res, 401, { error: 'unauthorized' });
    ...
    if (method === 'POST' && url === '/heartbeat') {
      ...
      if (!machineId && !deviceName) return json(res, 400, { error: 'machineId or deviceName required' });
      try {
        let existing = machineId ? await findDeviceByMachineId(String(machineId)) : null;
        if (!existing && deviceName) existing = await findDeviceByName(String(deviceName));
        if (!existing) return json(res, 404, { error: 'device not registered' });
    ...
    if (method === 'GET' && url === '/device/me') {
      ...
      if (!entry) return json(res, 404, { error: 'not registered yet' });
    ...
    if (method === 'POST' && url === '/register') {
      if (!registerLimiter.allow(clientIp(req), 10)) return json(res, 429, { error: 'too many registration attempts' });
      const body = await readJson(req);
      const { deviceName, machineId, port } = body;
      if (!deviceName) return json(res, 400, { error: 'deviceName required' });
      const bodyDeviceToken = body.deviceToken ? String(body.deviceToken) : '';
      const isDeviceChannel = isValidDeviceToken(bodyDeviceToken);
      const auth = await authOwnerOrDevice(req);
      if (!auth.owner && !isDeviceChannel) return json(res, 401, { error: 'unauthorized' });
      ...
      if (deviceToken) registerPendingDevice(String(deviceName), deviceToken); // 审批前轮询认证用
