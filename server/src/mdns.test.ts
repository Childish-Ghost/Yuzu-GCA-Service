...
function buildQuery(id: number, name: string, qtype = 12, qclass = 1): Buffer {
...
function encodeNameForTest(name: string): Buffer {
  const out: number[] = [];
  for (const label of name.replace(/\.$/, '').split('.')) {
    out.push(label.length);
    for (const b of Buffer.from(label)) out.push(b);
  }
  out.push(0);
...
function parseResponse(pkt: Buffer): { types: number[]; names: string[]; port?: number; target?: string; ttl: number } {
...
function decodeNameForTest(buf: Buffer, off: number): { name: string; next: number } {
  const labels: string[] = [];
  let pos = off;
  for (;;) {
    const len = buf[pos];
...
function queryWithNameBytes(nameBytes: number[], qtype = 12, qclass = 1): Buffer {
  const header = Buffer.alloc(12);
...
test('畸形：超长名称（累计 >255 字节）→ 不应答', () => {
  const bytes: number[] = [];
  for (let i = 0; i < 200; i++) bytes.push(2, 0x61, 0x62); // 200×3=600 字节标签流
