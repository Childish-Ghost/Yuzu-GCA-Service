...
import { push, sendFeishuCard, updateFeishuCard, OWNER_FEISHU_OPEN_ID } from './push.js';
...
const failedCodes = new Map<string, { count: number; ts: number }>();
/** 待注册设备 token 登记表（name → {token, expiresAt}） */
const pendingDevices = new Map<string, { token: string; expiresAt: number }>();
...
function toPublicOp(op: PendingOp) {
  return {
    id: op.id,
    device: op.device,
...
function cardSignature(opId: string): string {
  return createHmac('sha256', serverConfig.token || 'gca-no-token')
    .update(opId).digest('hex').slice(0, 16);
}

/** 校验卡片回调签名 + senderId 是 owner */
export function verifyCardAction(opId: string, signature: string, senderId: string): boolean {
  if (senderId !== OWNER_FEISHU_OPEN_ID) return false;
  return tokenEqual(signature, cardSignature(opId));
}

/** 构造审批交互卡片（schema 2.0）并推送飞书（2026-08-14：文本确认码 → 授权框卡片） */
async function pushApprovalCard(op: PendingOp): Promise<void> {
  ...
  if (r.ok && r.messageId) op.cardMessageId = r.messageId;
}

/** 审批后回写卡片状态 */
export async function updateApprovalCard(op: PendingOp, status: string): Promise<void> {
  if (!op.cardMessageId) return;
...
export function createOpRequest(
  device: string, operation: string, detail: string,
  deviceIp?: string, machineId?: string, devicePort?: number, deviceToken?: string
): { id: string; code: string; expiresInSec: number } {
...
export function getOpByCode(code: string): PendingOp | undefined {
  return [...ops.values()].find(o => o.code === code);
}

/** 按 id 审批（App/卡片回调通道，2026-08-14）——device_registration 副作用由调用方处理 */
export function approveOpById(id: string): { ok: boolean; op?: PendingOp; error?: string } {
  const op = ops.get(id);
  if (!op) return { ok: false, error: 'op not found' };
  // 审查 M2：先查终态（对已终态 op 报 already，而非 expired——语义正确）
  if (op.status !== 'pending') return { ok: false, error: `op already ${op.status}` };
...
export function rejectOpById(id: string): { ok: boolean; op?: PendingOp; error?: string } {
  const op = ops.get(id);
  if (!op) return { ok: false, error: 'op not found' };
  if (op.status !== 'pending') return { ok: false, error: `op already ${op.status}` };
  if (Date.now() - op.createdAt > CODE_TTL_MS) {
...
export function listOps(status?: string): ReturnType<typeof toPublicOp>[] {
  const out: ReturnType<typeof toPublicOp>[] = [];
...
export function getOpStatusPublic(id: string): {
  id: string; device: string; operation: string; status: string;
...
export function registerPendingDevice(name: string, token: string): void {
  pendingDevices.set(name, { token, expiresAt: Date.now() + PENDING_DEVICE_TTL_MS });
}

export function pendingDeviceNameByToken(token: string): string | null {
  const now = Date.now();
  for (const [name, p] of pendingDevices) {
    if (p.expiresAt > now && tokenEqual(p.token, token)) return name;
  }
  return null;
}

export function clearPendingDevice(name: string): void {
...
export function sweepOps(): number {
  ...
  if (failedCodes.size > 0) {
    for (const [code, f] of failedCodes) {
      if (now - f.ts > 60 * 60 * 1000) failedCodes.delete(code);
    }
  }
  // 待注册设备登记过期清扫
  for (const [name, p] of pendingDevices) {
