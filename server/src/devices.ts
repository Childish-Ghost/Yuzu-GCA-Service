...
let cachedConfig: OpenClawConfig = {};
...
export async function getDeviceEndpoint(name: string): Promise<{ url: string; auth: string } | null> {
  const config = await readConfig();
...
export async function findDeviceByMachineId(machineId: string): Promise<{ name: string; cfg: McpServerConfig } | null> {
  const config = await readConfig();
  const servers = config.mcp?.servers ?? {};
  for (const [name, cfg] of Object.entries(servers)) {
    if (cfg.machineId === machineId) return { name, cfg };
  }
  return null;
}

/** 按名称查设备（/heartbeat 兜底：配对注册未带 machineId 时按名称匹配，S10 修复） */
export async function findDeviceByName(name: string): Promise<{ name: string; cfg: McpServerConfig } | null> {
  const config = await readConfig();
  const server = config.mcp?.servers?.[name];
...
export async function findDeviceByToken(token: string): Promise<{ name: string } | null> {
  const config = await readConfig();
  const servers = config.mcp?.servers ?? {};
  for (const [name, cfg] of Object.entries(servers)) {
    if (cfg.deviceToken && tokenEqual(cfg.deviceToken, token)) return { name };
  }
  return null;
}
 ...
 */
export async function registerDevice(name: string, ip: string, port: number, deviceToken: string, machineId?: string): Promise<void> {
...
export async function updateDeviceToken(name: string, newToken: string): Promise<boolean> {
  if (!isValidDeviceToken(newToken)) return false;
...
export async function renameDevice(oldName: string, newName: string): Promise<boolean> {
  const ok = await withConfigLock(async () => {
    const config = await readConfig();
    const servers = config.mcp?.servers;
    if (!servers?.[oldName]) return false;
...
export async function updateDeviceUrl(name: string, newUrl: string): Promise<boolean> {
  const ok = await withConfigLock(async () => {
    const config = await readConfig();
    const server = config.mcp?.servers?.[name];
    if (!server) return false;
