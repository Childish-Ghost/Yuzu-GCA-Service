...
function decodeName(buf, off) {
    ...
    for (;;) {
        if (pos >= buf.length)
            throw new Error('name: offset past end');
        const len = buf[pos];
        if (len === 0) {
            pos += 1;
            if (!jumped)
        ...
        if ((len & 0xc0) === 0xc0) {
            if (pos + 1 >= buf.length)
                throw new Error('name: truncated pointer');
            const ptr = ((len & 0x3f) << 8) | buf[pos + 1];
            if (ptr >= pos)
                throw new Error('name: pointer not backward');
            if (++jumps > MAX_POINTER_JUMPS)
                throw new Error('name: too many pointer jumps');
            if (!jumped)
                end = pos + 2;
            jumped = true;
            pos = ptr;
        ...
        if (pos + 1 + len > buf.length)
            throw new Error('name: label past end');
        total += len + 1;
        if (total > MAX_NAME_BYTES)
            throw new Error('name: name too long');
...
export function buildResponse(id, port, info, ips) {
...
export function shouldRespond(pkt) {
    if (pkt.length < 12)
        return { id: 0, ok: false, unicast: false };
    const qd = pkt.readUInt16BE(4);
    const flags = pkt.readUInt16BE(2);
    if (qd === 0 || (flags & 0x8000) !== 0)
        return { id: 0, ok: false, unicast: false }; // 非查询/重复响应
 ...
 */
export function startMdnsAnnouncer(opts) {
    const log = opts.log ?? (() => { });
    ...
    const announce = () => {
        ...
        socket.send(pkt, MDNS_PORT, MDNS_GROUP, (err) => {
            if (err && !closed)
                log('[mdns] announce failed:', err.message);
        });
    };
    socket.on('message', (msg, rinfo) => {
        ...
        socket.send(pkt, destPort, destAddr, (err) => {
            if (err && !closed)
                log('[mdns] respond failed:', err.message);
        });
