...
const TOOLS = [
    {
        name: 'approve_op',
        description: 'Approve or reject a pending GCA operation (device registration, power, service) using its 6-digit confirmation code. ' +
    ...
    },
    {
        ...
        description: 'Request device registration. Generates a 6-digit confirmation code pushed to feishu/wechat. ' +
    ...
    {
        name: 'chat_ai',
        description: 'Send a message to the AI assistant and get its reply. ' +
...
async function handleToolsCall(id, params) {
    ...
    switch (name) {
        case 'approve_op': {
            ...
            if (!code)
                return { error: { code: -32602, message: 'code required' } };
            const action = String(args.action ?? 'approve');
            if (action === 'reject') {
                const ok = rejectOp(code);
                if (!ok)
                    return { error: { code: -32602, message: 'invalid or expired code' } };
            ...
            if (!result.ok || !result.op)
                return { error: { code: -32602, message: result.error || 'invalid or expired code' } };
            const op = result.op;
        ...
        case 'register_device': {
            ...
            if (!deviceName)
                return { error: { code: -32602, message: 'deviceName required' } };
            ...
            if (deviceToken)
                registerPendingDevice(deviceName, deviceToken);
        ...
        case 'revoke_device': {
            ...
            if (!name)
                return { error: { code: -32602, message: 'name required' } };
            const removed = await revokeDevice(name);
            if (!removed)
                return { error: { code: -32602, message: 'device not found' } };
        ...
        case 'push_message': {
            ...
            if (!text)
                return { error: { code: -32602, message: 'text required' } };
        ...
        case 'chat_ai': {
            ...
            if (!message)
                return { error: { code: -32602, message: 'message required' } };
