...
function healthUrl(base, portOffset) {
    try {
...
export function createEventService(deps) {
    ...
    function applyResult(dev, which, probe) {
        const svc = dev[which];
        if (probe.ok) {
    ...
    async function probeRound() {
        await syncRegistry();
        await Promise.allSettled([...state.values()].map(async (dev) => {
            ...
            if (!beforeAny && afterAny)
                broadcast('device.online', frame(dev));
            else if (beforeAny && !afterAny)
                broadcast('device.offline', frame(dev));
        ...
        if (round % 6 === 0) {
            for (const dev of state.values()) {
                if (dev.agent.online || dev.term.online)
                    broadcast('device.updated', frame(dev));
            }
        }
    }
    ...
    return {
        ...
        async handleEvents(req, res) {
            ...
            const sub = { write: (c) => res.write(c), end: () => res.end() };
            subscribers.add(sub);
            const drop = () => {
        ...
        notifyHeartbeat(name, newUrl) {
            ...
            if (!st)
                return; // 未入表（首次注册等）——下一轮 syncRegistry 自愈
            if (st.url !== newUrl) {
                st.url = newUrl;
                broadcast('device.updated', frame(st));
