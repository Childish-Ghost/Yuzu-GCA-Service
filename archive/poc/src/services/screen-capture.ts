...
function runPowerShell(script: string, env: Record<string, string> = {}, timeoutMs = 20000): Promise<string> {
...
export async function captureScreen(quality = 70): Promise<ScreenCaptureResult> {
  const out = await runPowerShell(PS_CAPTURE, { GCA_JPEG_QUALITY: String(quality) }, 20000);
  const parsed = JSON.parse(out.trim()) as { b64: string; w: number; h: number; left: number; top: number };
...
export async function ocrImage(jpegBase64: string): Promise<string> {
  try {
 ...
 * Timeout: 30s.
 */
export async function captureScreenAndroid(quality: number): Promise<ScreenCaptureResult> {
  ...
  try { await unlink(requestFile); } catch {}
  ...
  while (Date.now() < deadline) {
    ...
    try {
      ...
      if (errMsg) {
        try { await unlink(resultFile.replace('.png', '.error')); } catch {}
        throw new Error(`Android screenshot failed: ${errMsg}`);
      }
    } catch (err) {
      if (err instanceof Error && err.message.startsWith('Android screenshot failed')) throw err;
  ...
  throw new Error(
    'Screenshot timed out (30s). Ensure Accessibility Service is enabled: ' +
