...
import { getClipboardData, setClipboardData, type ClipboardData } from './clipboard.js';
...
let lastLocal: ClipboardData = { type: 'text', content: '' };
let lastRemoteUpdatedAt = 0;
let running = false;

function sameContent(a: ClipboardData, b: ClipboardData): boolean {
...
async function syncLoop(): Promise<void> {
  ...
  while (running) {
    ...
    try {
      ...
      if (res.ok) {
        const remote = await res.json() as { content: string; type?: string; deviceId: string; updatedAt: number };
        const remoteData: ClipboardData = { type: (remote.type as 'text' | 'image') || 'text', content: remote.content };
