...
export type PendingOperation =
  | { kind: 'exec'; command: string; cwd?: string; timeout?: number; background?: boolean }
  | { kind: 'file_write'; path: string; content: string; mode: 'overwrite' | 'append'; createDirs: boolean }
  | { kind: 'file_move'; source: string; dest: string }
  | { kind: 'file_delete'; path: string; recursive: boolean }
  | { kind: 'file_serve'; path: string }
  | { kind: 'file_fetch'; url: string; targetPath: string }
  | { kind: 'screenshot'; quality: number; ocr: boolean }
  | { kind: 'screen_consent'; minutes: number }
  | { kind: 'remote_input'; inputAction: import('./input-simulator.js').InputAction }
  | { kind: 'input_consent'; minutes: number }
  | { kind: 'clipboard_sync'; action: 'get' | 'set'; text: string }
  | { kind: 'power'; action: PowerAction; delaySec?: number; mac?: string }
  | { kind: 'service'; action: 'start' | 'stop' | 'restart'; name: string };
...
function isExpired(entry: PendingApproval, ttlMs: number): boolean {
  return Date.now() - entry.createdAt > ttlMs;
}

function describeOperation(op: PendingOperation): string {
  switch (op.kind) {
    case 'exec':
...
export function createPending(
  input: { operation: PendingOperation; reason: string },
 ...
 */
export function consumeLatestOfKinds(
  kinds: PendingOperation['kind'][],
  filter?: (op: PendingOperation) => boolean,
  ttlMs: number = DEFAULT_TTL_MS,
): PendingApproval | null {
  let newestToken: string | null = null;
  let newest: PendingApproval | null = null;
  for (const [token, entry] of pending) {
    if (!kinds.includes(entry.operation.kind)) continue;
    if (filter && !filter(entry.operation)) continue;
    if (isExpired(entry, ttlMs)) continue;
 ...
 */
export function validateOtpNonce(
  code: string,
  ttlMs: number = DEFAULT_TTL_MS,
): PendingApproval | null {
  for (const [token, entry] of pending) {
    if (!entry.nonce || isExpired(entry, ttlMs)) continue;
    if (entry.nonce === code) {
      pending.delete(token);
      return entry;
    }
