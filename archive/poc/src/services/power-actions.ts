...
export async function executePowerAction(input: PowerActionInput): Promise<string> {
  ...
  switch (action) {
    case 'shutdown': {
      ...
      if (r.exitCode !== 0) throw new Error(r.stderr || `shutdown exited ${r.exitCode}`);
    ...
    case 'sleep': {
      ...
      const r = await executeCommand(cmd, { timeout: 10000 });
      if (r.exitCode !== 0) throw new Error(r.stderr || `sleep exited ${r.exitCode}`);
      return 'Sleep initiated';
    }
    case 'hibernate': {
      const cmd = isWindows ? 'shutdown /h' : 'systemctl hibernate';
      const r = await executeCommand(cmd, { timeout: 10000 });
      if (r.exitCode !== 0) throw new Error(r.stderr || `hibernate exited ${r.exitCode} (is hibernation enabled?)`);
      return 'Hibernation initiated';
    }
    case 'wol': {
      if (!mac) throw new Error('wol requires a mac parameter');
      await sendWol(mac);
    ...
    case 'abort': {
      ...
      const r = await executeCommand(cmd, { timeout: 10000 });
      if (r.exitCode !== 0) throw new Error(r.stderr || `abort exited ${r.exitCode} (no shutdown scheduled?)`);
