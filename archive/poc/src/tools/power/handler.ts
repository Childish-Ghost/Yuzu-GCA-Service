...
export async function powerHandler(args: PowerInput) {
  if (isAndroid) return { content: [{ type: 'text' as const, text: JSON.stringify({ status: 'unsupported', reason: 'power actions require DevicePolicyManager or root — not available in embedded Node.js on Android' }) }] };
  ...
  if (action === 'abort') {
    try {
      ...
      const body: PowerOkResult = { status: 'ok', action, detail };
      return {
  ...
  if (OTP_ACTIONS.has(action)) {
    ...
    const channel = await sendDesktopNotification(
      'GCA 高危操作验证码',
      `操作: ${action} | 验证码: ${confirmToken} | 5 分钟内有效。若非本人操作请忽略。`,
    );
