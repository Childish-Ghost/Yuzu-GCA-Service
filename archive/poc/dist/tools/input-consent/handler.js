...
export async function inputConsentHandler(args) {
    if (isAndroid)
        return { content: [{ type: 'text', text: JSON.stringify({ status: 'unsupported', reason: 'input_consent requires foreground Activity dialog — not available in embedded Node.js on Android' }) }] };
    const { minutes } = args;
