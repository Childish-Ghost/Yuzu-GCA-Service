...
import { consumePending, consumeLatestOfKinds, validateOtpNonce } from '../../services/pending-approvals.js';
...
async function runConfirmedPower(op) {
    try {
...
async function runConfirmedService(op) {
    try {
...
export async function confirmHandler(args) {
    ...
    if (!token) {
        const latest = consumeLatestOfKinds(['exec', 'file_write', 'file_move', 'file_delete', 'file_serve', 'file_fetch', 'screenshot', 'screen_consent', 'remote_input', 'input_consent', 'clipboard_sync', 'power'], (op) => op.kind !== 'power' || op.action === 'wol');
        if (latest) {
