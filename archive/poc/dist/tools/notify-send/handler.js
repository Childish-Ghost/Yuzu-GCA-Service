...
export async function notifySendHandler(args) {
    if (isAndroid)
        return { content: [{ type: 'text', text: JSON.stringify({ status: 'unsupported', reason: 'notify_send requires native notification API — use Kotlin NotificationManager on Android' }) }] };
    ...
    const body = { status: 'sent', channel, title };
    return {
