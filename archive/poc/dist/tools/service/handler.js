...
export async function serviceHandler(args) {
    if (isAndroid)
        return { content: [{ type: 'text', text: JSON.stringify({ status: 'unsupported', reason: 'service management requires systemd or Windows SCM — not available on Android' }) }] };
    ...
    const channel = await sendDesktopNotification('GCA 高危操作验证码', `操作: service ${action} ${name} | 验证码: ${confirmToken} | 5 分钟内有效。若非本人操作请忽略。`);
    const body = {
        status: 'confirmation_required',
        operation: 'service',
