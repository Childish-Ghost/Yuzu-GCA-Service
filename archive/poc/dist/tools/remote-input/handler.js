...
import { executeInput } from '../../services/input-simulator.js';
...
export async function remoteInputHandler(args) {
    if (isAndroid)
        return { content: [{ type: 'text', text: JSON.stringify({ status: 'unsupported', reason: 'remote_input requires AccessibilityService — not available in embedded Node.js on Android' }) }] };
    ...
    switch (action) {
        case 'mouse_move':
            if (x === undefined || y === undefined)
                return errorResult('mouse_move requires x and y');
        ...
        case 'key_type':
            if (!text)
                return errorResult('key_type requires text');
...
export async function executeRemoteInput(inputAction, confirmedByUser = true) {
    try {
