...
async function cmdStatus() {
    ...
    console.log(`totp:     ${(await isTotpProvisioned()) ? 'provisioned ✓' : 'not provisioned'}`);
...
async function cmdDoctor() {
    ...
    check('totp provisioned', await isTotpProvisioned());
    // relay reachability
    let relayOk = false;
    try {
...
async function cmdSetup() {
    ...
    console.log('next: npm run setup:pairing && npm run setup:totp');
}
// ---------- service (auto-start) ----------
function startupVbsPath() {
    return path.join(os.homedir(), 'AppData', 'Roaming', 'Microsoft', 'Windows', 'Start Menu', 'Programs', 'Startup', 'gca-server.vbs');
}
async function serviceStatus(verbose = true) {
    if (isWindows) {
        ...
        if (verbose)
            console.log(`auto-start: ${registered ? `registered ✓ ${detail}` : detail}`);
    ...
    if (verbose)
        console.log(`auto-start: ${registered ? 'enabled ✓ (systemd --user)' : 'not enabled'}`);
    ...
    pair: async () => {
        ...
        try {
            const res = await fetch(`${serverUrl}/pair/claim`, {
                ...
                body: JSON.stringify({ code, deviceName: config.deviceName, port: config.port }),
            ...
            // Save the pairing token
            await setSetting('security.mcpToken', data.pairingToken);
            console.log(`\n✅ Paired successfully!`);
