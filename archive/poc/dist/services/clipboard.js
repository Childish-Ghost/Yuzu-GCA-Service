...
function runPowerShell(script, env = {}, timeoutMs = 10000) {
    return new Promise((resolve, reject) => {
        const child = spawn('powershell.exe', ['-NoProfile', '-NonInteractive', '-OutputFormat', 'Text', '-Command', script], { env: { ...process.env, ...env, PYTHONIOENCODING: 'utf-8' }, stdio: ['ignore', 'pipe', 'pipe'], windowsHide: true });
        const chunks = [];
        let stderr = '';
...
function runShell(cmd, input, timeoutMs = 5000) {
    return new Promise((resolve, reject) => {
        ...
        child.on('close', (code) => {
            ...
            if (code === 0)
                ...
                reject(new Error(stderr.trim().substring(0, 300) || `shell exited ${code}`));
        });
        if (input)
...
export async function setClipboardData(data) {
    if (data.type === 'image') {
        ...
        if (bytes > MAX_IMAGE_BYTES)
            throw new Error(`Image too large (${bytes} bytes, cap is ${MAX_IMAGE_BYTES})`);
        if (isWindows) {
