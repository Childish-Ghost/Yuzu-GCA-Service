...
export async function powerHandler(args) {
    if (isAndroid)
        return { content: [{ type: 'text', text: JSON.stringify({ status: 'unsupported', reason: 'power actions not available on Android' }) }] };
    ...
    if (action === 'abort') {
        try {
            ...
            return { content: [{ type: 'text', text: JSON.stringify({ status: 'ok', action, detail }, null, 2) }] };
        }
        catch (err) {
            return { content: [{ type: 'text', text: JSON.stringify({ status: 'error', action, error: err instanceof Error ? err.message : String(err) }, null, 2) }], isError: true };
        }
    }
    // wol: needs mac, simple confirm
    if (action === 'wol') {
        if (!mac)
            return { content: [{ type: 'text', text: JSON.stringify({ status: 'error', action, error: 'wol requires a mac parameter' }, null, 2) }], isError: true };
        try {
    ...
    if (!gcaServer) {
        return { content: [{ type: 'text', text: JSON.stringify({ status: 'error', action, error: 'gca-server 未配置（GAP_RELAY_URL）' }, null, 2) }], isError: true };
    }
    try {
        ...
        return { content: [{ type: 'text', text: JSON.stringify({ status: 'error', action, error: data.error || 'ops request failed' }, null, 2) }], isError: true };
    }
    catch (err) {
        return { content: [{ type: 'text', text: JSON.stringify({ status: 'error', action, error: err instanceof Error ? err.message : String(err) }, null, 2) }], isError: true };
    }
