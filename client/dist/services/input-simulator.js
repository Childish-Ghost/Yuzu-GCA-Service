...
function runPowerShell(script, env = {}, timeoutMs = 10000) {
    return new Promise((resolve, reject) => {
        const child = spawn('powershell.exe', ['-NoProfile', '-NonInteractive', '-Command', script], { env: { ...process.env, ...env }, stdio: ['ignore', 'pipe', 'pipe'], windowsHide: true });
        let stdout = '';
        let stderr = '';
...
export async function executeInput(action) {
    if (!isWindows) {
...
async function executeInputLinux(action) {
    switch (action.type) {
        case 'mouse_move': {
            const r = await executeCommand(`xdotool mousemove ${action.x} ${action.y}`, { timeout: 5000 });
            if (r.exitCode !== 0)
                throw new Error(`xdotool mousemove failed: ${r.stderr}`);
        ...
        case 'mouse_click': {
            ...
            if (r.exitCode !== 0)
                throw new Error(`xdotool click failed: ${r.stderr}`);
        ...
        case 'mouse_scroll': {
            ...
            const r = await executeCommand(cmd, { timeout: 5000 });
            if (r.exitCode !== 0)
                throw new Error(`xdotool scroll failed: ${r.stderr}`);
        ...
        case 'key_type': {
            ...
            if (r.exitCode !== 0)
                throw new Error(`xdotool type failed: ${r.stderr}`);
            logger.info('text typed (linux)', { chars: action.text.length });
            return `typed "${action.text.substring(0, 50)}"`;
        }
    }
}
//# sourceMappingURL=input-simulator.js.map