...
export async function getPairingToken(): Promise<string | null> {
  ...
  const fromSettings = await getSetting<string>(TOKEN_SETTING_KEY);
 ...
 */
export function requirePairing(): express.RequestHandler {
...
export async function logPairingState(): Promise<void> {
  const token = await getPairingToken();
  if (token) {
