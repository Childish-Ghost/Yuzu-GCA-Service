...
export async function listServices(filter?: string, limit = 50): Promise<ServiceInfo[]> {
  if (os.platform() === 'win32') {
    ...
    if (r.exitCode !== 0) throw new Error(r.stderr.substring(0, 200));
    const raw = JSON.parse(r.stdout) as
      | { Name: string; DisplayName: string; Status: number | string }[]
      | { Name: string; DisplayName: string; Status: number | string };
    ...
    if (filter) {
      ...
      services = services.filter(
        (s) => s.name.toLowerCase().includes(needle) || s.displayName.toLowerCase().includes(needle),
      );
    }
...
export async function executeServiceAction(
  ...
  if (r.exitCode !== 0 || (os.platform() === 'win32' && !r.stdout.includes('OK'))) {
    throw new Error(
      (r.stderr || r.stdout || `exit ${r.exitCode}`).substring(0, 300) +
