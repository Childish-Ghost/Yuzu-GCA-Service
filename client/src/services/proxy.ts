 ...
 */
export async function getProxyForUrl(targetUrl: string): Promise<string | null> {
  let host = '';
  try {
  ...
  const fileSocks = await getSetting<string>('proxy.socks');
  const fileHttps = await getSetting<string>('proxy.https');
  const fileHttp = await getSetting<string>('proxy.http');
  const fileBypass = await getSetting<string[]>('proxy.bypass');
