...
import { consumePending, consumeLatestOfKinds, type PendingOperation } from '../../services/pending-approvals.js';
import type {
...
export async function confirmHandler(args: ConfirmInput) {
  ...
  if (!token) {
    const latest = consumeLatestOfKinds(
      ['exec', 'file_write', 'file_move', 'file_delete', 'file_serve', 'file_fetch', 'screenshot', 'screen_consent', 'remote_input', 'input_consent', 'clipboard_sync'],
    );
    if (latest) {
