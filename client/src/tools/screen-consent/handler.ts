...
export async function screenConsentHandler(args: ScreenConsentInput) {
  if (isAndroid) return { content: [{ type: 'text' as const, text: JSON.stringify({ status: 'unsupported', reason: 'screen_consent requires foreground Activity dialog — not available in embedded Node.js on Android' }) }] };
  const { minutes } = args;
