...
// --- confirm (generic approval dispatcher: exec / file_write / file_move) ---
...
export type FileWriteToolResult =
  | FileWriteOkResult
...
export type FileMoveToolResult =
  | FileMoveOkResult
  | WriteOpConfirmationRequiredResult
  | FileMoveErrorResult;

// --- shared: write operations requiring confirmation ---
...
export type FileDeleteToolResult =
  | FileDeleteOkResult
  | WriteOpConfirmationRequiredResult
  | FileDeleteErrorResult;
...
export type ExecBackgroundToolResult =
  | ExecBackgroundStartedResult
  | WriteOpConfirmationRequiredResult
  | ExecBlockedResult
  | ExecBackgroundErrorResult;

// --- power ---

/** node-only 的 'wol'（agent 侧无 wol——rust tools/power.rs 无此 action，C11 标注） */
...
// --- file_serve / file_fetch (cross-device transfer, C-007) ---
...
export type FileServeToolResult =
  | FileServeOkResult
  | WriteOpConfirmationRequiredResult
  | FileTransferErrorResult;

export type FileFetchToolResult =
  | FileFetchOkResult
  | WriteOpConfirmationRequiredResult
  | FileTransferErrorResult;
...
export interface SysinfoOkResult {
  ...
  disk:
    | { path: string; totalGB: number; freeGB: number; usedPercent: number | null }
    | { error: string };
  network: { name: string; address: string; mac: string }[];
  collectedAt: string;
}
